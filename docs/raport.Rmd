...
Na powyższych wykresach warto zwrócić uwagę na ostatni - dla współczynnika korelacji równego 1. W przedstawionym wypadku algorytm SALP osiąga praktycznie takie same wyniki jak Best Fit - wynika to z faktu, że dla współczynnika korelacji przebiegi mają taką samą serię wzrostów i spadków, różnią się jedynie skalą, co uniemożliwia optymalizowanie niezrównoważenia, co stara się robić algorytm SALP, przez co jednocześnie redukuje się wymiarowość problemu - w związku z brakiem różnic zmianach przebiegów wektorów obciążenia, w praktyce można je zredukować jedynie do sumarycznego lub średniego obciążenia, co w praktyce robi algorytm Best Fit.
...
W związku z tym, że w symulacjach były używane te same ziarna losowe generatora, ten wykres obrazuje wpływ transofrmacji powiazanych z korelacją na zmiany w generowanych rozkładach.
...
Powyższy wykres wskazuje na istnienie zależności pomiędzy współczynnikiem korelacji a współczynnikiem zmienności częśtości przedkłądania zadań. 
...
Jednocześnie obserwujemy brak zależności między obciążeniem a współczynnikiem zmienności częśtości przedkłądania zadań, wynika to z tego, że wartość obciążenia nie jest potrzebna ani na etapie generowania danych ani na etapie wykonywania algorytmów, przez co wartość obciążenia nie ma wpływu na ułożenie zadań.
...
Nowy Model Obciążeń charakteryzuje się tym, że do dobrego przydzielenia shardów, wymagane jest branie pod uwagę niezrównoważenia. Jasne więc jest, ze algorytm SALP, który bierze pod uwagę niezrównoważenie, osiąga znacząco lepszy rezultat niż konkurencja.
